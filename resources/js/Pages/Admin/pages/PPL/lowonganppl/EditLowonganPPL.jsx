...
export const EditLowonganPPL = ({ daftar_prodi, daftar_sekolah, lowongan }) => {
    const [name, setName] = useState(lowongan?.name);
    const [description, setDescription] = useState(lowongan?.description);
    const [qouta, setQouta] = useState(lowongan?.quota);

    const [daftarProdi, setDaftarProdi] = useState(daftar_prodi);
    const [daftarSekolah, setDaftarSekolah] = useState(daftar_sekolah);
    const [prodi, setProdi] = useState(
        daftarProdi?.find((itm) => itm.id === lowongan.id_prodi) || null
    );
    const [sekolah, setSekolah] = useState(
        daftarSekolah?.find((itm) => itm.id === lowongan.id_sekolah) || null
    );
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        post(`/admin/lowonganppl/update/${lowongan.id}`);
    };
...
export default EditLowonganPPL;
